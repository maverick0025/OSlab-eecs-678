...
static void *dp_thread(void *arg)
{
  ...
  while (!Stop) {
    ...
    while(!(*left_chop_available(me) && *right_chop_available(me))){
    	pthread_cond_wait(&(me->can_eat), &waiter); // Will have to wait for both chop sticks are available
    }
    ...
    *right_chop_available(me) = 0;
     ...
     *left_chop_available(me) = 1; //setting the value to 1 so that other threads would know the chopsticks are available. here chopsticks are locks.
